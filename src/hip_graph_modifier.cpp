...
class FusionGroup : public hipGraphNode {
 public:
  FusionGroup() : hipGraphNode(hipGraphNodeTypeKernel) {
    semaphore_ = PlatformState::instance().getSemaphore();
  }
  virtual ~FusionGroup() { delete fusedNode_; };
  void addNode(hipGraphKernelNode* node) { fusee_.push_back(node); }
  std::vector<hipGraphKernelNode*>& getNodes() { return fusee_; }
  hipGraphKernelNode* getHead() { return fusee_.empty() ? nullptr : fusee_.front(); }
  hipGraphKernelNode* getTail() { return fusee_.empty() ? nullptr : fusee_.back(); }

  hipGraphKernelNode* getFusedNode() { return fusedNode_; }
  void generateNode(void* functionHandle);
  hipKernelNodeParams* getNodeParams() { return &fusedNodeParams_; }

 private:
  hipGraphNode* clone() const override { return nullptr; }
  std::vector<hipGraphKernelNode*> fusee_{};
  hipGraphKernelNode* fusedNode_;
...
void FusionGroup::generateNode(void* functionHandle) {
  ...
  hipError_t status = hipGraphKernelNode::validateKernelParams(&fusedNodeParams_);
  if (hipSuccess != status) {
    ClPrint(amd::LOG_ERROR, amd::LOG_ALWAYS, "`validateKernelParams` during fusion");
  }
  fusedNode_ = new hipGraphKernelNode(&fusedNodeParams_);
}
}  // namespace
...
void GraphModifier::Finalizer::operator()(size_t* instanceCounter) const { delete instanceCounter; }

GraphModifier::GraphModifier(hipGraph_t& graph) : graph_(graph) {
...
std::vector<hipGraphNode*> GraphModifier::collectNodes(const std::vector<Node>& originalNodes) {
  size_t nodeCounter{0};
  std::vector<hipGraphNode*> nodes{};
  ...
  for (const auto& group : executionGroups) {
    if (group.size() == 1) {
      ...
      for (const auto& nodeNumber : group) {
        ...
        auto* kernelNode = dynamic_cast<hipGraphKernelNode*>(originalNode);
